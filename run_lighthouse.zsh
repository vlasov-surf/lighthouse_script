...
for scenario in "${scenarios[@]}"; do
  IFS="|" read -r url form_factor <<< "$scenario"
  if [[ -z "$url" ]]; then
  ...
  base_name="${current_date}_${current_time}_${url_slug}_${form_factor}"

  # Создаём пустой файл заголовков по умолчанию
  headers_file=$(mktemp)
  echo '{}' > "$headers_file"
  ...
  # Создаём для каждого прогона свежую папку профиля Chrome
  tmp_profile=$(mktemp -d)

  # Проверяем, содержит ли URL "vseinstrumenti.ru"
  if [[ "$url" == *"vseinstrumenti.ru"* ]]; then
...
EOF
  ...
  # Проверяем, содержит ли URL "lemanapro.ru"
  if [[ "$url" == *"lemanapro.ru"* ]]; then
    cat > "$headers_file" <<EOF
{
  "Cookie": "st_uid=fa5a35644025ed875437f1560494a22c; cookie_accepted=true; qrator_jsid2=v2.0.1751543590.035.d5817fcfkYPLpJU0|53BV6tPEVCjQSKPo|go/yvfer3PvylYUc3IZkrxKRKGSDDewWdI2fM0rrhrIk/WIqgNSaHbdP/M+uew7194izSh5t5IRjyOHYWV450pSyBkwLZ1z7Pmdi76Zqvj34jCFXEmVe/Bl/bsn2kZ7m/wiR4I6s5+UdrQji7YWGOw==-yTn9sBK9KBoXKJUQeC42upUQ2v8="
}
EOF
fi
